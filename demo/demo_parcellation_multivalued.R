...
vol_layers <- c(200, 100, 100)
common_layers <- c(250, 100, 200)
# individual_layers <- c(500, 200, 100)
number_of_outputs <- 1
...
model_parc <- create_and_compile_multivalued_model(width, 
                                                   vol_layers = vol_layers, 
                                                   common_layers = common_layers, 
                                                   num_values = num_classes + 1,
...
model_parc %>% fit_with_generator(generator = train_generator_array,
...
# new_seg <- new_model_parc %>% infer_volume_categorical(V = as.array(new_img), 
#                                                        scale = scale_type,
#                                                        max_batch_size = 50000)

new_seg <- model_parc %>% infer_volume(V = as.array(new_img), 
                                           scale = scale_type,
                                           max_batch_size = 50000, 
                                           binarise = FALSE, 
                                           fast = TRUE)

neurobase::ortho2(new_seg)
gc()
